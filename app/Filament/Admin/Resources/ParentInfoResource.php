...
class ParentInfoResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form->schema([
            ...
            Forms\Components\Section::make('Ayah')->columns(['default' => 1, 'md' => 2, 'xl' => 3])->schema([Forms\Components\TextInput::make('father_name')->label('Nama Ayah')->required()->columnSpan(['default' => 1, 'md' => 2]), Forms\Components\TextInput::make('father_nik')->label('NIK')->validationAttribute('NIK Ayah')->maxLength(16), Forms\Components\TextInput::make('father_occupation')->label('Pekerjaan'), Forms\Components\TextInput::make('father_phone')->label('Telepon')->tel(), Forms\Components\TextInput::make('father_income')->label('Penghasilan')->prefix('Rp')->numeric()]),
            Forms\Components\Section::make('Ibu')->columns(['default' => 1, 'md' => 2, 'xl' => 3])->schema([Forms\Components\TextInput::make('mother_name')->label('Nama Ibu')->required()->columnSpan(['default' => 1, 'md' => 2]), Forms\Components\TextInput::make('mother_nik')->label('NIK')->validationAttribute('NIK Ibu')->maxLength(16), Forms\Components\TextInput::make('mother_occupation')->label('Pekerjaan'), Forms\Components\TextInput::make('mother_phone')->label('Telepon')->tel(), Forms\Components\TextInput::make('mother_income')->label('Penghasilan')->prefix('Rp')->numeric()]),
            Forms\Components\Section::make('Wali')->collapsed()->columns(['default' => 1, 'md' => 2])->schema([Forms\Components\TextInput::make('guardian_name')->label('Nama Wali'), Forms\Components\TextInput::make('guardian_relationship')->label('Hubungan'), Forms\Components\TextInput::make('guardian_phone')->label('Telepon')->tel(), Forms\Components\Textarea::make('guardian_address')->label('Alamat')->columnSpanFull()]),
        ]);
    }

    public static function table(Table $table): Table
    {
        return $table->defaultSort('created_at', 'desc')->columns([Tables\Columns\TextColumn::make('registration.registration_number')->label('No. Registrasi'), Tables\Columns\TextColumn::make('registration.full_name')->label('Calon Siswa')->searchable(), Tables\Columns\TextColumn::make('registration.unit.name')->label('Unit')->badge(), Tables\Columns\TextColumn::make('father_name')->label('Ayah')->searchable(), Tables\Columns\TextColumn::make('mother_name')->label('Ibu')->searchable()])->actions([Tables\Actions\EditAction::make(), Tables\Actions\DeleteAction::make()]);
    }

    public static function getPages(): array
    {
        return ['index' => Pages\ListParentInfos::route('/'), 'create' => Pages\CreateParentInfo::route('/create'), 'edit' => Pages\EditParentInfo::route('/{record}/edit')];
    }

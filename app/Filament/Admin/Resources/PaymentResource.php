...
class PaymentResource extends Resource
{
    ...
    {
        return $form->schema([
            Forms\Components\Select::make('registration_id')->relationship('registration', 'registration_number')->label('Pendaftaran')->searchable()->preload()->required(),
            Forms\Components\TextInput::make('va_number')->label('VA')->required(),
            Forms\Components\TextInput::make('amount')->label('Nominal')->prefix('Rp')->numeric()->required(),
        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('proof_original_name')
                    ->label('Bukti')
                    ->url(fn (Payment $record): ?string => $record->proof_path
                        ? route('files.applicant.payments.proof', $record)
                        : null)
                    ->default('-'),
            ])
            ->filters([
            ...
            ->actions([
                Tables\Actions\Action::make('verify')
                    ...
                    ->visible(fn (Payment $record) => auth()->user()?->can('verify_payment_payment') && $record->status === 'paid')
    ...
    {
        return (string) static::getEloquentQuery()->where('status', 'paid')->count();
    }
}

<x-filament-panels::page>
    @php
        ...
        $verifiedDocuments = $registration->documents->where('is_verified', true)->pluck('type');
    @endphp
        ...
        @if ($registration->data_validation_status === 'revision')
            <x-filament::section icon="heroicon-o-exclamation-triangle" icon-color="warning">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-300">
                    {{ $registration->data_validation_notes ?: 'Tata Usaha meminta perbaikan data pendaftaran. Hubungi petugas untuk informasi lebih lanjut.' }}
                </p>
            </x-filament::section>
        @endif
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        @elseif ($registration->current_stage === 'payment_verification')
                            <x-filament::badge color="warning" icon="heroicon-m-clock">
                                Menunggu verifikasi pembayaran TU
                            </x-filament::badge>
                        ...
                        @elseif ($registration->current_stage === 'data_validation')
                            <x-filament::badge color="warning" icon="heroicon-m-clock">
                                Menunggu validasi data oleh TU
                            </x-filament::badge>
                        @elseif ($registration->current_stage === 'virtual_account')
                            <x-filament::badge color="warning" icon="heroicon-m-envelope">
                                Menunggu Virtual Account dari TU
                            </x-filament::badge>
                        @elseif ($registration->current_stage === 'applicant_card')
                            <x-filament::badge color="warning" icon="heroicon-m-identification">
                                Menunggu penerbitan kartu pendaftar
                            </x-filament::badge>
                        @elseif ($registration->current_stage === 'tests')
                            <x-filament::badge color="info" icon="heroicon-m-academic-cap">
                        ...
                        @elseif ($registration->current_stage === 'announcement')
                            <x-filament::badge color="warning" icon="heroicon-m-megaphone">
                ...
                @if ($registration->announcement?->status === 'published')
                    <x-filament::section icon="heroicon-o-megaphone" icon-color="success">
                        ...
                        <div class="prose max-w-none text-sm dark:prose-invert">
                            <p>{{ $registration->announcement->message }}</p>
                        </div>
                ...
                <x-filament::section>
                    ...
                    <dl class="space-y-4 text-sm">
                        <div>
                            ...
                            <dd class="mt-1 font-semibold text-gray-950 dark:text-white">{{ $registration->registration_number }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">Unit tujuan</dt>
                            <dd class="mt-1 font-medium text-gray-950 dark:text-white">{{ $registration->unit?->name ?? '-' }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">NIK calon siswa</dt>
                            <dd class="mt-1 font-medium text-gray-950 dark:text-white">{{ $registration->nik }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">Tanggal daftar</dt>
                            <dd class="mt-1 font-medium text-gray-950 dark:text-white">{{ $registration->submitted_at?->format('d M Y H:i') ?? $registration->created_at->format('d M Y H:i') }}</dd>
                        </div>
                    </dl>
                </x-filament::section>
                ...
                <x-filament::section>
                    ...
                    @if ($registration->latestPayment)
                        <dl class="space-y-3 text-sm">
                            <div>
                                <dt class="text-gray-500">Virtual Account</dt>
                                <dd class="mt-1 font-mono font-semibold text-gray-950 dark:text-white">{{ $registration->latestPayment->virtual_account ?? 'Belum tersedia' }}</dd>
                            </div>
                            <div>
                                <dt class="text-gray-500">Nominal</dt>
                                <dd class="mt-1 font-semibold text-gray-950 dark:text-white">Rp {{ number_format((float) $registration->latestPayment->amount, 0, ',', '.') }}</dd>
                            </div>
                            <div>
                                <dt class="text-gray-500">Status</dt>
                                <dd class="mt-1"><x-filament::badge>{{ str($registration->latestPayment->status)->replace('_', ' ')->title() }}</x-filament::badge></dd>
                            </div>
                        </dl>
                    @else

            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Satu akun orang tua dapat mendaftarkan lebih dari satu calon siswa.
                    </p>
                </div>
            ...
            <x-filament::section>
                <div class="py-8 text-center">
                    ...
                    <p class="mx-auto mt-2 max-w-xl text-sm text-gray-500 dark:text-gray-400">
                        Mulai dengan mengisi data calon siswa. Setelah tersimpan, progres validasi, pembayaran, dokumen, tes, seleksi, dan pengumuman dapat dipantau di sini.
                    </p>
                </div>
            </x-filament::section>
        @else
                    ...
                    <x-filament::section>
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <x-filament::button tag="a" href="{{ route('registration.show', $registration) }}" outlined>
                                    Lihat Progres
                                </x-filament::button>

                                @if (in_array($registration->current_stage, ['payment', 'payment_verification']))
                                    <x-filament::button tag="a" href="{{ route('registration.payment', $registration) }}" icon="heroicon-m-banknotes">
                                        Pembayaran
                                    </x-filament::button>
                                @endif

                                @if (in_array($registration->current_stage, ['documents', 'document_verification']))
                                    <x-filament::button tag="a" href="{{ route('registration.documents', $registration) }}" icon="heroicon-m-document-arrow-up">
                                        Lengkapi Dokumen
                                    </x-filament::button>
                                @endif

                                @if ($registration->applicant_card_number)
                                    <x-filament::button tag="a" href="{{ route('registration.card', $registration) }}" color="gray" icon="heroicon-m-identification">
                                        Kartu Pendaftar
                                    </x-filament::button>
                                @endif
                            </div>
